...
<section x-data="{ 
    ...
    " x-show="Toast.Show" x-transition:enter-start="top-enter" x-transition:enter-end="top-enter-end" x-transition:leave-start="top-leave" x-transition:leave-end="top-leave-end" style="left:50%;transform:translateX(-50%)" class="pos-fixed transition-all g-5px z-index-5000 top-20px br-5px row align-center p-10px p-x-15px  no-select no-pointer">
       <svg x-show="Toast.Type == 'success'" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 32 32">
  <g fill="currentColor">
    <path d="m16,1C7.729,1,1,7.729,1,16s6.729,15,15,15,15-6.729,15-15S24.271,1,16,1Zm-2.43,21.985l-5.984-5.985,1.414-1.414,4.43,4.429,9.437-11.423,1.542,1.273-10.838,13.119Z" stroke-width="0" fill="currentColor"></path>
  </g>
</svg>
<svg x-show="Toast.Type == 'info'" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 12 12">
  <g fill="currentColor">
    <path d="m6,0C2.691,0,0,2.691,0,6s2.691,6,6,6,6-2.691,6-6S9.309,0,6,0Zm.75,8.5c0,.414-.336.75-.75.75s-.75-.336-.75-.75v-3c0-.414.336-.75.75-.75s.75.336.75.75v3Zm-.75-4.5c-.482,0-.875-.393-.875-.875s.393-.875.875-.875.875.393.875.875-.393.875-.875.875Z" stroke-width="0" fill="currentColor"></path>
  </g>
</svg>
<span x-text="Toast.Message" class="ws-nowrap font-weight-800"></span>
    ...
    <div x-data="{  }" class="w-full column border-element br-20px row p-15px g-10px">
        ...
        <div class="row text-align-center align-center w-full g-10px space-between">
            ...
            Claiming = true;
            SendPostRequest('{{ url('users/post/daily/claim/process') }}',{
                '_token' : '{{ @csrf_token() }}'
            },function(response,error){
               
                let data= JSON.parse(response);
                Toast.Type=data.status;
                Toast.Message = data.message;
                Toast.Show = true;
                Claiming = false;

            })
            " x-bind:class="Claiming ? 'no-pointer' : ''" class="w-full column g-5px border-width-1px border-style-solid border-color-primary-05 bg-primary-005 p-10px align-center justify-center br-10px">
             <i class="c-primary">
                 <svg x-show="!Claiming" width="30" height="30" viewBox="0 0 24 24" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
