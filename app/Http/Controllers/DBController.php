...
class DBController extends Controller
{
    ...
    if(!Schema::hasColumn('users','api_token')){
        Schema::table('users',function($table){
                $table->json('api_token')->after('status')->nullable()->comment('api token json');
        });
    }

    if(!Schema::hasTable('tracklist')){
        Schema::create('tracklist',function($table){
            $table->id();
            $table->string('uniqid');
            $table->string('banner')->nullabel()->comment('Display Banner');
            $table->string('audio')->nullable()->comment('Track Audio');
            $table->string('name')->nullable()->comment('Track Name');
            $table->string('artist')->nullable()->comment('Artist/Musician who sang the song');
            $table->bigInteger('reward')->nullable()->comment('Streaming Reward');
            $table->bigInteger('streams')->default(0)->comment('Total streams');
            $table->string('status')->default('active');
            $table->timestamp('updated')->useCurrent();
            $table->timestamp('date')->useCurrent();
        });
    ...
    if(!Schema::hasTable('streams')){
        Schema::create('streams',function($table){
            ...
            $table->json('track')->comment('The track streamed as a json');
            $table->string('status')->default('success');
            $table->timestamp('updated')->useCurrent();
            $table->timestamp('date')->useCurrent();
        });
