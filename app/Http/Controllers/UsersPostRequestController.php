...
class UsersPostRequestController extends Controller
{
    ...
    public function Register(){
        ...
        $email=trim(strtolower(strtolower(request('email'))));
        $phone=request('phone');
...
$id=GenerateID();
      ...
      DB::transaction(function() use($amount,$proof,$id,$receipt,$bank){
        DB::table('users')->where('id',Auth::guard('users')->user()->id)->update([
    ...
    'balance' => [
        'before' => Auth::guard('users')->user()->deposit_balance,
        'after' => Auth::guard('users')->user()->deposit_balance
    ],
    'primary_wallet' => 'Deposit Wallet'
    ...
    public function CompleteTask(){
            ...
            DB::transaction(function() use($task,$proof,$status,$reward){
    ...
    public function Withdrawal(){
        ...
        if($amount < $minimum){
            return response()->json([
                'message' => 'Minimum withdrawal for '.$wallet_name.' = '.Auth::guard('users')->user()->currency.number_format($minimum,2),
                'status' => 'error'
            ]);
        }
        if($amount > $maximum){
