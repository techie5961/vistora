...
// pending
    ...
    $each->ref_id=DB::table('users')->where('username',$each->ref)->first()->id ?? null;
    return $each;
        });
    ...
    $user->last_deposit=DB::table('transactions')->where('user_id',$user->id)->where('type','deposit')->where('status','success')->orderBy('date','desc')->first()->amount ?? 0;
     $user->ref_id=DB::table('users')->where('username',$user->ref)->first()->id ?? null;
    ...
    public function ManageTasks(){
        ...
        $tasks->getCollection()->transform(function($each){
                    $each->category=json_decode($each->category);
                    $each->status=$each->completed >= $each->slots ? 'completed' : 'active';
                    $each->frame=Carbon::parse($each->date)->diffForHumans();
                    return $each;
        });
        return view('admins.tasks.manage',[
            ...
            'total_active' => DB::table('tasks')->whereColumn('completed','<','slots')->count(),
            'total_completed' => DB::table('tasks')->whereColumn('completed','>=','slots')->count()
        ]);
    }
    ...
    public function EditTask(){
        ...
        $task->category=json_decode($task->category);
    ...
    public function SubmittedTaskProofs(){
        ...
        if(request()->has('user_id')){
            $proofs=$proofs->where('user_id',request('user_id'));
        }
